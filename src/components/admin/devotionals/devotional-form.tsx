...
type Props = {
  ...
  submitLabel: string;
  onSubmit: (input: DevotionalInput) => Promise<{
    ok: boolean;
    error?: string;
...
export function DevotionalForm({
  ...
  const t = useTranslations("devotionals.form");
  const tCommon = useTranslations("common");
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  const form = useForm<FormValues>({
    resolver: zodResolver(devotionalInputSchema as never),
  ...
  function handleSubmit(values: FormValues) {
    startTransition(async () => {
      const result = await onSubmit({
        title: values.title,
        verseRef: values.verseRef || undefined,
        verseText: values.verseText || undefined,
        body: values.body,
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("fields.verseRef")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    placeholder={t("fields.verseRefPlaceholder")}
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("fields.authorName")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("fields.verseText")}</FormLabel>
              <FormControl>
                <Textarea
                  {...field}
                  rows={4}
                  placeholder={t("fields.verseTextPlaceholder")}
                />
              </FormControl>
              <FormDescription>{t("fields.verseTextHint")}</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
